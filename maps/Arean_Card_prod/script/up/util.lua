...
function up.set_mouse_wheel_switch(value)
    gameapi.get_all_role_ids().set_role_mouse_wheel(value)
end

function up.get_game_mode_id()
...
function up.splitStringByFlag(str,flag)
    local splitStr = {}
    while true do
        local a,b = string.find(str,flag)
        if a == nil then
...
function up.get_localplayer()
    if GameAPI.get_owner_role_id() and 1 == up.player(GameAPI.get_owner_role_id())._base:get_role_status() then
        return up.player(GameAPI.get_owner_role_id())
    else
        for index, role_iter in python.enumerate(gameapi.get_all_role_ids()) do
            if (1 == gameapi.get_role_by_role_id(role_iter).get_role_status()) then
                return up.player(role_iter)
            end
		end
    end
end