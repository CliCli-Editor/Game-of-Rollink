...
function up.unit_class.__index.add_effect(self,data)
    ...
    if not data.visible_type then data.visible_type = 1 end
    effect._base = GameAPI.create_projectile_on_socket(data.id, self._base, data.socket, data.angle, data.target, data.skill, data.visible_type)
    setmetatable(effect, mt)
    if data.life then
        effect:set_life(data.life)
    end
    if data.debug_unit then
        print(data.debug_unit._base)
        print(data.debug_unit._base,data.debug_unit:get_owner())
        print(data.debug_unit._base,data.debug_unit:get_owner(),up.player(self._base:api_get_role():api_get_role_id()))
        print(data.debug_unit._base,data.debug_unit:get_owner(),up.player(self._base:api_get_role():api_get_role_id()),up.player(data.target:api_get_role():api_get_role_id()))
    end
