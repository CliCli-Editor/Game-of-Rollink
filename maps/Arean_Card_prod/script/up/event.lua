...
local event_list = {
   ...
   -- {EVENT.SELECT_UNIT_GROUP,           'Unit-MutiSelect',{{'__role_id','player'},{'__unit_group_id_list','group'},'__team_id'}},
    ...
    {EVENT.ROLE_ACTIVE_EXIT_GAME_EVENT, 'Player-ExitGame',{{'__role_id','player'}}},
    {EVENT.CHAT_SEND_GM,                'Player-ChatSend',{{'__role_id','player'},'__str1'}},
    ...
    {EVENT.MOUSE_MOVE_EVENT,            'Mouse-Move',{{'__role_id','player'},},true},
    ...
    {EVENT.ROLE_JOIN_BATTLE,            'Player-JoinGame',{{'__role_id','player'}}},
    ...
    {EVENT.UI_VX_EVENT,                  'VX-Event',{'__ui_vx_handler','__comp_name','__int1'}}
}
...
function up.game:ui_event(name)
    
    local event = {{EVENT.TRIGGER_COMPONENT_EVENT,name},'UI-Event',{{'__role_id','player'},'__ui_event_name','__comp_name'}}
    local trg = new_global_trigger(100000+n, event[2], event[1], true)
    n = n+1

    --print('ui_event注册',name)
    trg.on_event = function(trigger,event_name,actor,data)
