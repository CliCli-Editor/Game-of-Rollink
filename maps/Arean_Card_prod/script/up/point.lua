...
function mt:offset(angle,distance)
    local x, y = self.x, self.y
    return up.actor_point(GlobalAPI.get_point_offset_vector(Fix32Vec3(x, self.z, y),Fix32(angle),Fix32(distance)))
end
...
up.actor_point = function(...)
    ...
    if n == 1 then
        ...
        if type(base) == 'number' then
            base = GameAPI.get_point_by_res_id(base)
        end
        point.x = GlobalAPI.get_fixed_coord_index(base,0):float()/100
        point.y = GlobalAPI.get_fixed_coord_index(base,2):float()/100
        point.z = GlobalAPI.get_fixed_coord_index(base,1):float()/100
    --get x,y
    elseif n == 2 then
        point.x, point.y = ...
        point.z = 0
        base = p2p(point.x/100,point.y/100,0)
    --get x,y,z
    elseif n == 3 then
        point.x, point.y, point.z = ...
        base = p2p(point.x/100,point.y/100,point.z/100)
    end
